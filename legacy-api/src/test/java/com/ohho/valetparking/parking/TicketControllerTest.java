...
/**
 * @WebMvcTest의 경우 자동으로 configuration하는 빈들은 생성한다. 이외에 다른 것들은 scan이 불가능하다. WebMvcconfiguer을 구현시켜 만든
 * 인터셉터를 위한 interceptorConfiguration이 존재합니다. 그렇게 되면 scan하여 빈을 생성합니다. 하지만 인터셉터에 등록되어 사용되는
 * AuthLoginInterceptor의 경우 @Component가 등록되어 있기 때문에 WebMvcTest가 스캔하지 못합니다.
 **/
@WebMvcTest(controllers = {TicketController.class}
    , excludeAutoConfiguration = SecurityAutoConfiguration.class
    , excludeFilters = {
    @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = WebMvcConfigurer.class)})
// spring security에서 제외 시키기 위한 작업
public class TicketControllerTest {
  ...
  void settingJwt() {
    ...
    ReflectionTestUtils.setField(testJwtProvider, "SECREAT_KEY",
  ...
  void apiresponseTest() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.get("/ticket/test/3")
        ...
        .andExpect(content().string("{\"localDateTime" + "\":\"" + LocalDateTime.now()
