...
import com.ohho.valetparking.global.error.exception.NotFoundTokenException;
...
/**
 * 레디스의 키값으로 저장되는 타입입니다. spring:session:sessions:expires (string) 해당 세션의 만료키로 사용합니다.
 * spring:session:expirations (set) expire time에 삭제될 세션 정보를 담고 있습니다. spring:session:sessions (hash)
 * session은 map을 저장소로 사용하기 때문에 이곳에 세션 데이터가 있습니다.
 * <p>
...
public class SessionUtil {
  ...
  public static void removeToken(HttpSession session) {
    Optional.ofNullable(getRefreshtoken(session))
        .orElseThrow(() -> new NotFoundTokenException(ErrorCode.INVALID_TOKEN));
